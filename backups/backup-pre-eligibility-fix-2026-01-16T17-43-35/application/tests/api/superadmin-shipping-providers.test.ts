/**
 * Auto-generated API Tests for /superadmin/shipping-providers
 * Generated: 2026-01-08T09:42:34.386Z
 */
...
const routePath = '/superadmin/shipping-providers'
const routeName = 'superadmin_shipping-providers'
...
export async function runTests(): Promise<TestResult[]> {
  ...
  try {
    ...
    // ===== GET /superadmin/shipping-providers =====
    ...
    try {
      ...
      const request = createMockRequest('GET', routePath,
        { query: {
          ...
          "providerRefId": "test-value",
          "includeInactive": "test-value"
} })
    ...
    try {
      const invalidRequest = createMockRequest('GET', routePath, {
        query: { providerId: 'invalid-id' },
        body: { providerCode: 'invalid-value' }
      })
    ...
    try {
      ...
      const objectIdRequest = createMockRequest('GET', routePath, {
        query: { providerId: '507f1f77bcf86cd799439011' },
        body: { providerCode: '507f1f77bcf86cd799439011' }
      })
    ...
    // ===== POST /superadmin/shipping-providers =====
    ...
    try {
      ...
      const request = createMockRequest('POST', routePath,
        { body: {
          "providerCode": "test-value",
          "providerName": "Test Name",
          "providerType": "test-value",
          "isActive": "test-value",
          "supportsShipmentCreate": "test-value",
          "supportsTracking": "test-value",
          "supportsServiceabilityCheck": "test-value",
          "supportsCancellation": "test-value",
          "supportsWebhooks": "test-value",
          "apiBaseUrl": "test-value",
          "apiVersion": "test-value",
          "authType": "test-value",
          "documentationUrl": "test-value",
          "authConfig": "test-value",
          "// Optional authentication configuration\r\n      createdBy": "test-value",
          "providerId": "test-value",
          "...updates": "test-value"
} })
    ...
    // ===== PUT /superadmin/shipping-providers =====
    ...
    try {
      // Test with ObjectId-like string (should be rejected or converted)
      const objectIdRequest = createMockRequest('PUT', routePath, {
        query: { providerId: '507f1f77bcf86cd799439011' },
        body: { providerCode: '507f1f77bcf86cd799439011' }
      })
      const response = await routeHandler.PUT(objectIdRequest)
      const responseData = await response.json()
      
      // Should either reject ObjectId or convert to string ID
      const passed = response.status >= 400 || (responseData && typeof responseData.id === 'string' && /^\d{6}$/.test(responseData.id))
      addResult(results, createTestResult('PUT', 'String ID validation', passed,
        passed ? undefined : 'Route should reject ObjectId or convert to string ID'))
    } catch (error: any) {
      addResult(results, createTestResult('PUT', 'String ID validation', true))
    }
    
    // Test: PUT - No ObjectId fallback
    // Route does not use ObjectId fallback (verified in route map)

    // ===== DELETE /superadmin/shipping-providers =====
    ...
    try {
      // Test with ObjectId-like string (should be rejected or converted)
      const objectIdRequest = createMockRequest('DELETE', routePath, {
        query: { providerId: '507f1f77bcf86cd799439011' },
        body: { providerCode: '507f1f77bcf86cd799439011' }
      })
      const response = await routeHandler.DELETE(objectIdRequest)
      const responseData = await response.json()
      
      // Should either reject ObjectId or convert to string ID
      const passed = response.status >= 400 || (responseData && typeof responseData.id === 'string' && /^\d{6}$/.test(responseData.id))
      addResult(results, createTestResult('DELETE', 'String ID validation', passed,
        passed ? undefined : 'Route should reject ObjectId or convert to string ID'))
    } catch (error: any) {
      addResult(results, createTestResult('DELETE', 'String ID validation', true))
    }
