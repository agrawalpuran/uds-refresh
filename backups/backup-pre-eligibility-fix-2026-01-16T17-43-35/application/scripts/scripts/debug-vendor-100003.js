/**
 * Debug script for vendor 100003
 * Check ProductVendor relationships and inventory records
 */
...
async function debugVendor100003() {
  try {
    ...
    // Step 1: Find vendor 100003
    console.log('📋 STEP 1: Finding vendor 100003...')
    const vendor = await db.collection('vendors').findOne({ id: '100003' })
    ...
    // Step 5: Analyze inventory records
    ...
    inventoryProducts.forEach((p, idx) => {
      ...
      console.log(`   ${idx + 1}. ${p.name} (SKU: ${p.sku}, id: ${p.id}, _id: ${p._id?.toString()}) - ${status}`)
    })
    
    // Step 6: Check each inventory record
    console.log(`\n📋 STEP 6: Detailed inventory record analysis:`)
    allInventory.forEach((inv, idx) => {
      const product = inventoryProducts.find(p => p._id.toString() === inv.productId?.toString())
      const isAssigned = productIds.some(pid => pid.toString() === inv.productId?.toString())
      console.log(`   ${idx + 1}. Inventory ID: ${inv.id}`)
      console.log(`      Product: ${product?.name || 'UNKNOWN'} (${product?.sku || 'N/A'})`)
      console.log(`      Product _id: ${inv.productId?.toString() || 'N/A'}`)
      console.log(`      Assigned: ${isAssigned ? '✅ YES' : '❌ NO'}`)
      console.log(`      Total Stock: ${inv.totalStock || 0}`)
    })
    
    // Step 7: Identify orphaned inventory
    console.log('\n📋 STEP 7: Identifying orphaned inventory records...')
    ...
    if (allInventory.length === pvLinks.length) {
      console.log(`\n✅ Inventory count matches ProductVendor relationships (${allInventory.length} = ${pvLinks.length})`)
    } else {
      console.log(`\n⚠️  Inventory count (${allInventory.length}) does NOT match ProductVendor relationships (${pvLinks.length})`)
    }
