...
export async function updateMasterOrderStatus(orderId: string | mongoose.Types.ObjectId): Promise<void> {
  ...
  if (process.env.DUAL_WRITE_ENABLED === "true") {
    try {
      ...
      const currentOrder = await Order.findOne({ id: orderIdStr }).select('status unified_status').lean()
      const oldStatus = currentOrder?.status || null
      const currentUnified = (currentOrder as any)?.unified_status || null
      
      const dualWriteResult = safeDualWriteOrderStatus(orderIdStr, derivedStatus as UnifiedOrderStatus, oldStatus, currentUnified, { source: 'updateMasterOrderStatus-indent' })
      
      const result = await Order.updateOne(
        { id: orderIdStr },
        { $set: { status: derivedStatus, ...dualWriteResult.unifiedUpdate } }
      )
      
      console.log(`[updateMasterOrderStatus] 🔄 DUAL-WRITE: Order ${orderIdStr} → status=${derivedStatus}`)
      
      if (result.matchedCount === 0) {
        console.error(`[updateMasterOrderStatus] Order not found: ${orderIdStr}`)
      }
    } catch (dualWriteError: any) {
      console.error(`[updateMasterOrderStatus] ❌ DUAL-WRITE error (falling back to legacy): ${dualWriteError.message}`)
      // FALLBACK: Legacy update
      await Order.updateOne({ id: orderIdStr }, { $set: { status: derivedStatus } })
    }
  } else {
